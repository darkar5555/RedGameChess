...
io.on('connection', function(socket) {
    ...
    socket.on('move piece', function(data){
		console.log(currentPlayer.name + ' bcst: moving spawn: ' + JSON.stringify(data));
		if(data.position[0] > 9 || data.position[1] > 9 || data.position[2] > 9){
			data.position[0] = data.position[0] / 10;
			data.position[1] = data.position[1] / 10;
			data.position[2] = data.position[2] / 10;
		}
	...
	socket.on('select piece', function(data){
		console.log(currentPlayer.name + 'primero bcst: selecting spawn: ' + JSON.stringify(data));
		if(data.position[0] > 9 || data.position[1] > 9 || data.position[2] > 9){
			data.position[0] = data.position[0] / 10;
			data.position[1] = data.position[1] / 10;
			data.position[2] = data.position[2] / 10;
		}
		console.log(currentPlayer.name + ' bcst: selecting spawn: ' + JSON.stringify(data));
		//console.log(data.position[0]);
		socket.emit('select piece', data);
		socket.broadcast.emit('select piece', data);
	});

	socket.on('stop clock', function(data){
		//console.log(currentPlayer.name + ' bcst: stoping clock: ' + JSON.stringify(data));
		//console.log(data.position[0]);
		
		socket.emit('stop clock', data);
		socket.broadcast.emit('stop clock', data);
	});
	socket.on('stop clockB', function(data){
		//console.log(currentPlayer.name + ' bcst: stoping clockBlack: ' + JSON.stringify(data));
		//console.log(data.position[0]);
		
		socket.emit('stop clockB', data);
		socket.broadcast.emit('stop clockB', data);
	});
